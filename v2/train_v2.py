...
def parse_args():
    p = argparse.ArgumentParser(description="BraTS 2024 Attention U-Net + Strong Augmentation (v2)")
    p.add_argument("--data_dir",      default="/storage/sahar/CV_project/data/train",
                   help="Directory of processed .h5 files")
    p.add_argument("--ckpt_dir",      default="/storage/sahar/CV_project/checkpoints_v2",
                   help="Checkpoint directory (val_split.json saved here too)")
    p.add_argument("--init_features", type=int,   default=32,
                   help="Base filter count — 16 locally, 32 on RunAI A100")
    p.add_argument("--epochs",        type=int,   default=300)
    p.add_argument("--batch_size",    type=int,   default=1)
    p.add_argument("--lr",            type=float, default=1e-4)
    p.add_argument("--dropout",       type=float, default=0.2)
    p.add_argument("--val_split",     type=float, default=0.2,
                   help="Val fraction — ignored if val_split.json already exists in ckpt_dir")
    p.add_argument("--num_workers",   type=int,   default=4)
    p.add_argument("--seed",          type=int,   default=42)
    p.add_argument("--patience",      type=int,   default=20,
                   help="Early-stopping patience (epochs without val Dice improvement)")
    p.add_argument("--resume",        action="store_true",
                   help="Resume from latest_checkpoint.pth in ckpt_dir")
